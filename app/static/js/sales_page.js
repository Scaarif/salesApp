...
function  App() {
    ...
    Recat.useEffect(() => {
        getData();
    }, [page]); //page is the dependency in this case - the api will be called everytime it changes
    ...
    return(
        <div>
                ...
                <div className="modal-dialog shadow">
                    ...
                    <div className="modal-body">
                        ...
                        <div className="form-group">
                            ...
                            id="itemInput" value={item} onChange={(e)=> 
                        ...
                        <div className="form-group">
                            ...
                            {setQuantity(e.target.value)}} placeholder="Quantity" />
                        </div>
                        <small className="form-text text-muted">{error}</small>
                    </div>
                    <div className="modal-footer">
                        <button type="button" className="btn btn-secondary"
                        onClick={() => {setShowModal(false)}} data-bs-dismiss="modal"
                        >Close</button>
                        <button type="submit" className="btn btn-primary" onClick={
                            saveOrder}>Save Changes</button>
                    </div>
                    </div>
                    </form>
                </div>
            ...
            marginTop: "1em", marginBottom: "1em", padding: "1em"}}
            className="shadow">
        ...
        <div style={{ display: "flex", flexDirection: "row", marginBottom:"5px"}}>
            {pages.length > 0 && <nav className="d-lg-flex 
            justify-content-lg-end dataTables_paginate paging_simple_numbers">
                <ul className="pagination">
                    <li className={"page-item" + (page===0 ? "disabled":"")}
                    onClick={(e)=> {e.preventDefault();
                    setPage(Math.max(page-1,0));}}><a className="page-link"
                    href="#" aria-label="Previous">
                        <span aria-hidden="true">«</span></a></li>
                    {pages.map((el) => <li key={'page'+ el.page} onClick=
                    {(e)=>{setPage(el.page);
                    }} className={'page-item'+(page===el.page?'active':'')}>
                        <a className="page-link" href="#">{el.name}</a></li>)}
                    <li className={"page-item" + (page ===pages.length-1 ? "disabled" : "")}
                        onClick={(e) => {
                ...
                <a className="btn btn-light" style={{marginLeft:  "auto"}}
                onClick={newOrder}>New Sales Order</a>
        </div>
