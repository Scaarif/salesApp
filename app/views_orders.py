...
def serialize_order(order):
	#create a dictionary from the database row(object)
...
def order(request, order_id):
	...
	try:
		...
		return HttpResponse(json.dumps({'detail': 'Not found'}),
			status = status.HTTP_404_NOT_FOUND)
	if request.method == 'GET':
		return HttpResponse(json.dumps({'data': serialize_order(order)}),
...
# move the data from the request to the order (for PUT and POST methods)
def save_order(request, order, success_status):
	...
	try:
		...
		if price == '':
			erros.append({'price': 'This field is required'})
		else:
			price = int(price)
			if price < 0:
...
def orders(request):
	...
	if request.method == 'GET':
		...
		orders_data = [serialize_order(order) for order in orders_data] #i.e a list

		return HttpResponse(json.dumps({'count': orders_count, 'data': orders_data}),
			status = status.HTTP_200_OK)
